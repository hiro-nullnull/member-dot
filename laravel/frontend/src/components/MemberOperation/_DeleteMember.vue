  ...
  <div class="deleteMember">
        ...
        >
          ...
          <div class="deleteMember__listContent">
            <div
              ...
              :key="index"
            >
              <div class="deleteMember__memberCard" @click="openDialog(member)">
                <vc-member-icon
    ...
    >
      <div class="deleteMember__confirmDialogBody">
        <vc-member-icon :icon-number="selectedMemberIconName"></vc-member-icon>

        <p class="deleteMember__confirmDialogBodyTitle">
          <span v-if="!isDeleted">{{ selectedMemberName }} を けす？</span>
          <span v-else class="nes-text is-error"
            >{{ selectedMemberName }} を けした</span
          >
        </p>

        <div class="deleteMember__confirmDialogBodyButtons">
          <template v-if="!isDeleted">
            <button @click="isShowDialog = false" type="button" class="nes-btn">
              やめる
            </button>
            <button
              @click="deleteMember()"
          ...
          <button v-else @click="isShowDialog = false" class="nes-btn">
            とじる
          </button>
        </div>
...
export default {
  ...
  methods: {
    openDialog(member) {
      ...
      this.selectedMemberIconName = member.icon_name;
      this.selectedMemberType = member.member_type;
      this.isShowDialog = true;
    },
    deleteMember() {
      const url = window.location.origin + "/member-management/web-api/delete";
      this.axios.put(url, { memberId: this.selectedMemberId }).then(res => {
        if (res.data.status === 0) {
          this.isDeleted = true;
          store.commit("filterUpdateMemberList", {
